...
public class HtmlParser {
	...
	public List<Offer> parseOffer(Document doc) {
		...
		for (int i = 1; i < rows.size(); i++) {
			...
			offer.setPrice(Float.parseFloat(this.getText(row, "span.olpOfferPrice").replaceAll("[$￥]", "")));
			System.out.println(offer.getPrice());
			String shippingFeeText = this.getText(row, "span.olpShippingPrice").replaceAll("[$￥]", "");
	...
	public Condition parseCondition(Element row) {
		...
		if(array.length > 1){
			condition.setSecondary(array[1].trim());// array[1] may not exist
		}
		return condition;
	}
